...
@interface CMExtractEntitiesApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation CMExtractEntitiesApi

NSString* kCMExtractEntitiesApiErrorDomain = @"CMExtractEntitiesApiErrorDomain";
NSInteger kCMExtractEntitiesApiMissingParamErrorCode = 234513;
...
///
///  @returns CMExtractEntitiesResponse*
///
-(NSURLSessionTask*) extractEntitiesPostWithValue: (CMExtractEntitiesRequest*) value
    ...
    if (value == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kCMExtractEntitiesApiErrorDomain code:kCMExtractEntitiesApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/nlp-v2/extract-entities"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"CMExtractEntitiesResponse*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((CMExtractEntitiesResponse*)data, error);
                                }
                            }];
}
