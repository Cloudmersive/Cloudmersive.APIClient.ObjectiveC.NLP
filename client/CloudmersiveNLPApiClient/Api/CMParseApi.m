...
@interface CMParseApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation CMParseApi

NSString* kCMParseApiErrorDomain = @"CMParseApiErrorDomain";
NSInteger kCMParseApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) parseParseStringWithInput: (CMParseRequest*) input
    ...
    if (input == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kCMParseApiErrorDomain code:kCMParseApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/nlp-v2/parse/tree"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"CMParseResponse*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((CMParseResponse*)data, error);
                                }
                            }];
}
